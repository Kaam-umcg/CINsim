...
append_cinsim <- function(karyoSim1 = NULL, karyoSim2 = NULL) {
  ...
  if(simplified) {
    ...
    gen_measures <- bind_rows(karyoSim1$gen_measures,
                              karyoSim2$gen_measures %>%
                                filter(g != 0) %>%
                                mutate(g = new_g2))

    # compile into a single object
    karyoSim <- list(karyotypes = karyoSim2$karyotypes,
    ...
    pop_measures <- bind_rows(karyoSim1$pop_measures,
    ...
    misseg_freq <- bind_rows(karyoSim1$misseg_freq,
