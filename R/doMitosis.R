...
doMitosis <- function(karyoMat, pMissegs = NULL) {
  ...
  pMissegsMatrix <- matrix(pMissegs, byrow = TRUE, nrow = nrow(karyoMat),
  ...
  daughterMat <- matrix(NA, nrow = nrow(karyoMat)*2, ncol = ncol(karyoMat),
                        dimnames = list(rep(rownames(karyoMat), 2),
                                        colnames(karyoMat)))
